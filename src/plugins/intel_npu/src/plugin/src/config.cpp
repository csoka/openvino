...
bool PluginConfig::hasOpt(std::string_view key) const {
    return _desc->has(key);
}

details::OptionConcept PluginConfig::getOpt(std::string_view key) const {
    return _desc->get(key);
}

bool PluginConfig::isOptPublic(std::string_view key) const {
...
void PluginConfig::update(const ConfigMap& options, OptionMode mode) {
    auto log = Logger::global().clone("Config");
...
bool PluginConfig::isAvailable(std::string key) const {
...
void PluginConfig::enable(std::string key, bool enabled) {
    // we insert for all cases - no need to check if exists
    _enabled[key] = enabled;
}

void PluginConfig::enableAll() {
    _desc->walk([&](const details::OptionConcept& opt) {
        enable(opt.key().data(), true);
    });
}

void PluginConfig::walkEnables(std::function<void(const std::string&)> cb) const {
    for (const auto& itr : _enabled) {
        cb(itr.first);
    }
}

void PluginConfig::walkInternals(std::function<void(const std::string&)> cb) const {
    for (const auto& itr : _internal_compiler_configs) {
        cb(itr.first);
    }
}

void PluginConfig::addOrUpdateInternal(std::string key, std::string value) {
    auto log = Logger::global().clone("Config");
...
std::string PluginConfig::getInternal(std::string key) const {
    auto log = Logger::global().clone("Config");
    if (_internal_compiler_configs.count(key) == 0) {
...
std::string PluginConfig::toStringForCompilerInternal() const {
...
std::string PluginConfig::toStringForCompiler() const {
    std::stringstream resultStream;
