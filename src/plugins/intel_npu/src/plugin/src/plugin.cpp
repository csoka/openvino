 ...
 */
void set_batch_config(bool isBatchingSupported, PluginConfig config) {
...
static ov::intel_npu::CompilerType resolveCompilerType(const PluginConfig base_conf, const ov::AnyMap& local_conf) {
...
void Plugin::recheck_compiler_support(PluginConfig& cfg) const {
    bool legacy = false;
    ...
    try {
        ...
        compiler = compilerAdapterFactory.getCompiler(_backends->getIEngineBackend(), cfg.get<COMPILER_TYPE>());
    } catch (...) {
...
PluginConfig Plugin::fork_local_config(const std::map<std::string, std::string>& rawConfig,
    ...
    PluginConfig localConfig = _globalConfig;
...
std::shared_ptr<ov::ICompiledModel> Plugin::import_model(std::istream& stream, const ov::AnyMap& properties) const {
    ...
    // ov::internal::cached_model_buffer has no corresponding "Config" implementation thus we need to remove it from
